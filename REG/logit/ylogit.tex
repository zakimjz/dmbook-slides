...
Figure shows the plot for the logistic function
    for $z$ ranging from $-\infty$ to $+\infty$. In particular
    consider what happens when $z$ is $-\infty$, $+\infty$ and $0$; we
    have
    \begin{align*}
    ...
    \end{align*}
    As desired, $\theta(z)$ lies in the range $[0,1]$, and $z=0$ is
    the ``threshold'' value in the sense that for $z > 0$ we have
    $\theta(z) > 0.5$, and for $z < 0$, we have $\theta(z) < 0.5$. 
    Thus, interpreting $\theta(z)$ as a probability, the larger the $z$
...
%\label{sec:reg:logit:MLE}
%
%\index{MLE|see{maximum likelihood estimation}}
%\index{maximum likelihood estimation}
%\index{logistic regression!maximum likelihood estimation}
...
However, for the log-likelihood formulation %in
...
\Repeat{$\norm{\abw^{\;t}-\abw^{\;t-1}} \le \epsilon$}{
    ...
    \ForEach{$\abx_i \in \abD$ in random order}{%
        ...
        \tcp*[h]{compute gradient at $\abx_i$}\;
...
%    \label{ex:reg:logit:logit}
%    \cref{fig:reg:logit:iris-vir:logit} 
...
%    
    ...
    otherwise we predict $\hy=0$. %\cref{fig:reg:logit:iris-vir:logit}
\end{frame}
...
\end{frame}
%
%It is also interesting to note that the choice of the reference class is
%not important, since we can derive the log-odds ratio for any two
%classes $c_i$ and $c_{\!j}$ as follows:
%\begin{align*}
...
{%
    \lForEach{$j = 1, 2, \cdots, K-1$}{
        $\abw_{\!j} \gets \abw^{\;t}_{\!j}$ \tcp*[h]{make a copy of
        $\abw^{\;t}_{\!j}$}
    }
    \ForEach{$\abx_i \in \abD$ in random order}{%
...
%%\psPoint(0.75,0.5,2.6){p1}
%%\uput[l](p1){$\pi_1$}
%%\psPoint(-3,-3,2.6){p2}
%%\uput[l](p2){$\pi_2$}
%%\psPoint(-0.5,-0.5,1.5){p3}
%%\uput[l](p3){$\pi_3$}
%%% \rput(-4,4.5){$\pi_1$}
%%%\rput(-0.5,4.25){$\pi_3$}
%%%\rput(5,4){$\pi_2$}
...
%% \psSurface[ngrid=.2 .2,incolor=white,axesboxed,
%%     linewidth=0.5\pslinewidth, 
%% algebraic,Zmin=0,Zmax=1](-4,-3)(4,2){0.5}
...
\framesubtitle{Example}
    ...
    shown in the figure.
%\cref{fig:reg:logit:iris-3K-multiclass}. 
Here, the response
