...
\begin{frame}{Linear Activation Function}
...
\begin{frame}{Step Activation Function}
...
\begin{frame}{Rectified Linear Activation Function}
...
\begin{frame}{Hyperbolic Tangent Activation Function}
...
%\begin{figure}[!b]
%    \centering
\begin{frame}{Softmax Activation Function}
...
\begin{frame}{Error Functions}
    ...
    {\bf Cross-Entropy Error:} For classification tasks, with $K$
        classes $\{c_1, c_2, \cdots, c_K\}$, %we usually set 
...
{\bf Binary Cross-Entropy Error:} For %classification tasks with
    binary classes, %it is typical to encode 
    the positive class is $1$
    and the negative class is $0$. %, as opposed to using a one-hot
    %encoding as in the general $K$-class case. 

	\medskip

Given %an input 
	$\bx \in \setR^d$, with %true response 
...
The partial derivative %of the binary cross-entropy error function 
...
results in %the following learned parameters, 
%$b = 0.0096$, $w_1 = -0.087$ and $w_2 = 0.452$, yielding the regression model
\begin{align*}
...
Therefore, each %input point 
$\bx_i$ is 2D, and 2-dimensional, and
%the true response vector 
$\by_i$ is also 2D.%-dimensional. That is, $d=2$
%and $p=2$ specify the size of the input and output layers. 
Minimizing the %squared error 
SSE via gradient descent, yields:% the following parameters:
\begin{align*}
    \matr{%
        b_1 & b_2\\
        w_{11} & w_{12}\\
...
\end{align*}
The SSE %squared error 
on the training set is $84.9$. Optimal %least squared
multivariate regression yields an %squared error 
SSE of $84.16$: %with the following parameters
\begin{align*}
    \matr{\hy_1 \\ \hy_2} =
    \matr{
...
\begin{frame}{Classification via Neural Networks}
...
\end{align*}
%Thus, with sigmoid activation, 
The output of the neural network is %in
...
\framesubtitle{Example}
    We applied the single output neural network %presented,
...
\begin{frame}{Neural networks for multiclass logistic regression}
\framesubtitle{Example}
We do not constrain the weights and bias for $o_3$ we obtain the
following model:
    \begin{align*}
...
Misclassified points are shown in dark gray color. Points in class
$c_1$ and $c_2$ are shown displaced with respect to the base class $c_3$
only for illustration.
%
%\begin{figure}[t!]
...
\begin{frame}{MLP: Feed-forward Phase}
...
neuron values, %we compute 
the value for each output
neuron $o_{\!j}$ is: %as follows:
\begin{align*}
...
\begin{frame}{MLP: Backpropagation Phase}
%
...
$o_{\!j}$. %This can also be computed via the chain rule
\begin{align*}
    \delta_{\!j} = \frac{\partial \cE_\bx}{\partial \net_{\!j}} =
...
%
%\enlargethispage{1\baselineskip}
%To compute the net gradient 
$\delta_{\!j}$ at %the hidden neuron 
$z_j$ has %we have 
...
To find the net gradient at $z_{\!j}$ we have to consider
the net gradients at each of the output neurons $\delta_k$ but weighted
by the strength of the connection $w_{\!jk}$ between $z_{\!j}$ and
$o_k$.%, as illustrated in \cref{fig:reg:neural:backpropOH}. 
...
%Figure shows 
	...
	The training data %(the gray points on the curve), which 
...
We evaluate an MLP with one hidden layer for the 
...
%We use 
%$n_l$ to denote the number of neurons in layer $l$.
...
%Let us consider how to compute the net gradients at the output layer $h+1$. 
...
\framesubtitle{Example}
    ...
    %During the training of each of the deep MLPs, 
We evaluate the 
...
\framesubtitle{Prediction error as a function of epochs.}
    The deeper MLP improves the prediction accuracy, but
 adding more layers does not reduce the error rate, and can
